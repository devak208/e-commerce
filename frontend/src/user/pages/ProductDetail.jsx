...
export default function ProductDetail() {
  ...
  // Generate product images (in a real app, you'd have multiple product images)
// Generate product images using the "image" field from the product
let productImages = []

if (product && product.image) {
  try {
    // Attempt to parse product.image as a JSON array
    const imagesArr = typeof product.image === "string" && product.image.trim().startsWith("[")
      ? JSON.parse(product.image)
      : [product.image]  // If not JSON, assume it's a single filename
      productImages = imagesArr.map(filename => `${import.meta.env.VITE_API_URL}/uploads/${filename}`)
  } catch (e) {
    // If parsing fails, treat it as a single image
    productImages = [`${import.meta.env.VITE_API_URL}/uploads/${product.image}`]
  }
} else {
